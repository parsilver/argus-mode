...
The same precondition covers the diff's secret-scan: the brief must attach the Stage-4 secret-scan output — a maintained scanner (gitleaks/trufflehog), or the shipped regex-sweep fallback when none is installed (`verification.md`, "what a failable check is"). **No secret-scan output attached is a refusal**, named the same way ("no secret-scan output attached"). A scan showing a hit is not a blocker to refuse on — it is a dimension-6 finding you report.
