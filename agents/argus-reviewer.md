...
| 3 | Architecture fit | Layer boundaries respected; single responsibility held; the diff doesn't reach across a boundary it shouldn't; reinvention unjustified in the plan — a helper, utility, or pattern the repo already provides, reimplemented — is a finding (fold into the existing code, or the plan records why not) |
