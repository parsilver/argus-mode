...
| v0.10.0 | structure validation pass at 1cf63e4 — 0 critical, 0 major (112 assertions pass / 0 fail / 3 n/a); `bash tests/run-checks.sh` 335 ok, 0 FAIL, reproduced from a clean worktree at that SHA — validated one commit before the tag, which differs only by this RELEASE-CHECKLIST.md row · skill review re-run this cycle — both `SKILL.md` files changed (+30/-13 and +148/-19), so the v0.8.0/v0.9.0 skip justification did not hold: two blocking findings fixed in the release PR, eight non-blocking filed as #115 | #1 live — this cycle's runs (#106 / PR #111) and this release's own pipeline; #2–#6 pending (the roll-up and the two fixes change no behavior surface those tests cover) |
| v0.11.0 | structure validation pass at de89e75 — 0 critical, 0 major (36 assertions pass / 0 fail); `bash tests/run-checks.sh` 433 ok, 0 FAIL, reproduced from a clean worktree at that SHA — validated the roll-up commit, which differs from the tagged tree only by this RELEASE-CHECKLIST.md row, which run-checks does not count · skill review re-run this cycle — both `SKILL.md` files changed (via #124 and #128): no blocking findings, three non-blocking filed as #131 | #1 live — this cycle's runs (#127 / PR #128) and this release's own pipeline; #2–#6 pending, and #4/#5 pending against a changed surface — #123's unconditional-worktree change touched the no-remote path and the Resume path those two tests cover, so the "roll-up changes no surface" justification does not hold for them this cycle |
