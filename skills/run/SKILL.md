...
3. Branch named `<n>-<slug>`, always in its own isolated worktree branched from `origin/<default>` (`git worktree add <path> -b <n>-slug origin/<default>`) — unconditional, and never `git switch` or a fast-forward inside the primary checkout. The mechanical in-flight probe (`pipeline.md`, git intake step 3) still runs as inventory: it feeds the in-flight announcement and Resume, not a checkout choice. No remote → the worktree branches off the local default tip (named degrade); no `gh` or no remote also skips the draft-PR probe arm (named, not silent).
