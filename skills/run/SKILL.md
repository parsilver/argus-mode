...
**Untrusted-input scan and trust tier:** the plan header carries two more lines, from the intake scan and probe above (`pipeline.md`, Untrusted input at intake) — `Untrusted-input scan: <sources> — <disposition>` and `Trust tier: <@author> — <level> (<probe evidence>) — <ratified|UNRATIFIED>`. On a run with nothing fetched they record the absence (`none fetched — no issue, PR, or comment text this run did not author` / `operator (in-session) — ratified by construction`). The plan-review gate checks both (rubric item 2), and criteria still reading `UNRATIFIED` do not become the contract until the operator ratifies them.
