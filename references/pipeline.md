...
| Issue type | Probe `repository.issueTypes` via GraphQL; null means the repo has no types — an organization-level feature, so user-owned repos always return null and take the named degrade below. | Set Bug / Feature / Task by the work's nature: `gh issue create --type` / `gh issue edit --type` (gh 2.95.0+), or GraphQL `updateIssueIssueType` with the type id; neither available → named degrade. |
