...
- Any missing → **announce which agents are absent now**, plainly. That direct announcement is the floor — it covers the read-only route and a skills-only install, where the capability preflight is never reached. On the full pipeline the same modes are re-shown alongside the environment capabilities when the run's shape is **announced in the capability preflight** at intake (`pipeline.md`, Capability preflight). Never a silent degrade.
