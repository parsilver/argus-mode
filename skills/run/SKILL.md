...
- Any missing → record which agents are absent; their resulting mode is **announced in the capability preflight** at intake (`pipeline.md`, Capability preflight), together with the environment capabilities — one consolidated announcement, not a separate one here. Never a silent degrade.
