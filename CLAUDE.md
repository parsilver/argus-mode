...
- **Every merged PR that changes shipped files** (`skills/`, `agents/`, `references/`, `.claude-plugin/`) **records the change under `## [Unreleased]` in `CHANGELOG.md`, in that same PR.** A **release PR** then moves the Unreleased entries into a new `## [x.y.z]` section and bumps `.claude-plugin/plugin.json` to match — the two always move together. Plugin updates are detected by version comparison only, so Unreleased changes on `main` are invisible to installed users until a release PR lands: that is deliberate batching, and the Unreleased section is what makes it visible instead of accidental.
- After each release merge, tag `v<version>` on the merge commit and create the GitHub Release matching the CHANGELOG entry. Never tag or announce content whose version wasn't bumped.
